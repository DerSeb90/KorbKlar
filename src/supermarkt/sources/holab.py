...
from ..common import build_match_key, clean_text, format_validity, normalize_pack, parse_base_price_text, parse_iso_date, parse_number
...
class OfficialHolabSource:
    ...
    def load(self,postal_code:str)->list[Offer]:
        ...
        for index,card in enumerate(page.select("li.offer"),1):
            ...
            prefix=clean_text(price_node.select_one(".price-tag__prefix").get_text(" ",strip=True) if price_node and price_node.select_one(".price-tag__prefix") else "");minimum=2 if re.search(r"ab\s*2\s*kisten",f"{prefix} {details}",re.I) else None;deposit_match=re.search(r"\+\s*(\d+[.,]\d{2})\s*Pfand",details,re.I);deposit=parse_number(deposit_match.group(1)) if deposit_match else None;base_price,base_unit=parse_base_price_text(details);image=card.select_one("img[src]");image_url=urljoin(self.BASE,clean_text(image.get("src"))) if image else "";offer_id=f"holab:{index}:{re.sub(r'[^a-z0-9]+','-',name.casefold()).strip('-')}";pack=normalize_pack(details)
