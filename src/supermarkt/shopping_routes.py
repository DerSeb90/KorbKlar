...
* ``/results/{search_id}/shopping-list/...`` for the browser interface and the
  app, protected by the same HMAC result token that already guards result data
  and the image proxy. Without a valid results link nobody can write to the
  list.

Without ``SUPERMARKT_KITCHENOWL_URL`` and ``SUPERMARKT_KITCHENOWL_TOKEN`` the
routes answer ``configured: false`` and the interface hides its controls.
"""
...
class ShoppingListItem(BaseModel):
    ...
    retailer: str = Field(default="", max_length=80, description="Händler, wird Kategorie oder Notiz")
...
@router.post(
    ...
    description=(
        ...
        "Der Artikelname ist das Produkt, die Notiz enthält Preis, Packung und Gültigkeit."
    ),
)
