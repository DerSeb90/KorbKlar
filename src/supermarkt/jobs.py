...
class SearchJobStore:
    ...
    def start(self, postal_code: str, refresh: bool = False) -> str:
        ...
        with self._lock:
            ...
            self._jobs[job_id] = {"job_id": job_id, "status": "waiting", "postal_code": postal_code,
                ...
                "step": "Angebote werden neu geladen" if refresh else "Suche wird vorbereitet",
                "progress": 0, "processed_sources": 0,
                "total_sources": 6, "processed_products": 0, "created_at": now, "updated_at": now}
        self._pool.submit(self._run, job_id, postal_code, refresh)
        return job_id
    ...
    def _run(self, job_id: str, postal_code: str, refresh: bool = False) -> None:
        try:
            snapshot, from_cache = self.engine.snapshot(postal_code, "auto", refresh,
