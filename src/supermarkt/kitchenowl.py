...
def _is_offer_headline(name: str) -> bool:
    ...
    Such entries exist because a shopping list may have received offers under
    their full advertised name. Left in the running they win every time, being
    the longest match for the very offer that created them, and the
    household's own "Brötchen" never gets used again.
    """
...
class KitchenOwlShoppingList:
    def __init__(
        ...
        # urlopen also opens file:/ and ftp:/. A mistyped
        # SUPERMARKT_KITCHENOWL_URL would then read local files instead of
        # making a request, so the integration counts as unconfigured.
    ...
    def _call(self, path: str, payload: dict[str, Any] | None = None) -> Any:
        ...
        try:
            # The scheme is limited to http/https in the constructor, so file:/
            # and ftp:/ never reach this line.
    ...
    def _category_id(self, household_id: str, retailer: str) -> int | None:
        ...
        if not clean_text(retailer):
            return None
        # Environment values arrive stripped, so the space between an emoji
        # prefix and the retailer is supplied here rather than expected.
        name = f"{self.category_prefix.strip()} {clean_text(retailer)}".strip()
        with self._lock:
    ...
    def health(self) -> dict[str, Any]:
        # The token is deliberately absent: /health is reachable without
        # authorisation so the container healthcheck works.
        return {
