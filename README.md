...
KorbKlar is a self-hosted service for comparing current regional supermarket offers in Germany. A German postal code is enough for normal use. The service discovers matching retailers, retrieves the available weekly offers, normalizes product names, package sizes and unit prices, and compares matching offers across retailers.

## Why KorbKlar exists

KorbKlar started as a vibe-coding project for a very specific personal workflow. The original plan was to let a local LLM trigger the comparison automatically and post the result through Conduit on Monday mornings, so the new weekly offers would already be available at the start of the week.

During development it became clear that the comparison engine works better as an independent service. Without an LLM in the critical path, KorbKlar is faster and easier to automate, while still being usable from a browser, scripts, REST clients or an LLM. For this use case, the standalone design is more flexible than the original LLM-only approach.

An LLM is therefore optional. KorbKlar can still be integrated into agent, Conduit or OpenAPI workflows, but the comparison itself does not depend on a model.
...
Open `http://SERVER-IP:8000`, enter a German postal code and start the search. No account, API key or LLM is required for normal browser use.

## Supported retailers

KorbKlar currently contains data paths for REWE, EDEKA, Marktkauf, ALDI Nord, ALDI Süd, Kaufland, Lidl, PENNY, Netto Marken-Discount and GLOBUS. Availability depends on the postal code, region and currently reachable source data.

## Main features

- regional retailer discovery by German postal code
- price and unit-price comparison
- package-size normalization
- optional loyalty prices
- retailer and product filters
- SQLite snapshot cache
- local image cache and SSRF protection
- browser UI and REST endpoint
- Docker deployment without an external database
- no mandatory LLM or cloud model

## REST API

Automations and external clients can use:
...
The REST endpoint can optionally be protected with `SUPERMARKT_API_KEY`. See [`.env.example`](.env.example) for the complete optional configuration.

## Development

KorbKlar requires Python 3.12 or newer for a native installation.
...
Live retailer tests are opt-in:
...
Retailer websites and undocumented web interfaces can change. Individual adapters may therefore fail temporarily while the rest of the service continues to work. KorbKlar does not invent missing prices or estimate loyalty discounts that are not explicitly provided by the source data.

## Support the project / Projekt unterstützen

Wer KorbKlar und die Arbeit von Tarnkappe.info unterstützen möchte, findet die aktuellen Spendenmöglichkeiten hier:

https://tarnkappe.info/spenden/

Die aktuell veröffentlichte Monero-Adresse von Tarnkappe.info lautet:

```text
87oownPVNFFciRNe2DLaNQPrVsjprbZBX7bHnPENABDyGpM6isrzKeWGsjT5W86h9d6A5nhE2Z2ZAXCHksvC2EikGGaWB8u
```

Wallet-Adressen können sich ändern. Vor einer Spende bitte die Adresse auf der verlinkten Tarnkappe.info-Spendenseite noch einmal prüfen.

If you want to support KorbKlar and the work of Tarnkappe.info, please use the donation page above and verify the current wallet address there before sending funds.
...
KorbKlar ist unabhängig und weder mit den genannten Händlern noch mit deren Kunden- oder Bonusprogrammen verbunden. Händler-, Produkt- und Programmnamen gehören den jeweiligen Rechteinhabern.
